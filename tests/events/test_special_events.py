"""
Test module for special events functionality.
"""
...
from src.events.special_events import SpecialEvents
from src.utils.persistence.db_provider import db_provider

class TestSpecialEvents(unittest.TestCase):
    """Test cases for SpecialEvents class."""
    ...
    def setUp(self):
        ...
        # Create a mock event
        self.event = SpecialEvents(self.bot)
        
        # Mock database provider
        self.db_patcher = patch('src.events.special_events.db_provider')
    ...
    def test_create_special_event_embed(self):
        """Test creating special event embed."""
        event_data = {
            'name': 'Test Event',
        ...
        embed = self.event.create_special_event_embed(event_data)
        self.assertIsInstance(embed, discord.Embed)
        self.assertEqual(embed.title, "🎉 Test Event")
        self.assertIn("Test Description", embed.description)
    
    def test_create_special_event_view(self):
        """Test creating special event view."""
        event_data = {
            'name': 'Test Event',
            'description': 'Test Description',
            'start_time': datetime.now(),
        ...
        view = self.event.create_special_event_view(event_data)
    ...
    def test_handle_special_event_registration(self):
        """Test handling special event registration."""
        # Mock player data
        player_data = {
        ...
        self.event.handle_special_event_registration(self.interaction, event_data)
        self.interaction.response.defer.assert_called_once()
        self.interaction.followup.send.assert_called_once()
    
    def test_send_special_event_announcement(self):
        """Test sending special event announcement."""
        # Mock channel
        ...
        message = self.event.send_special_event_announcement()
        self.assertEqual(message, self.message)
        self.channel.send.assert_called_once()
        self.assertTrue(self.message.pin.called)
    
    def test_cleanup_special_event(self):
        """Test cleaning up special event."""
        # Mock message
        self.message.delete = AsyncMock()
        
        # Test cleanup
        self.event.cleanup_special_event(self.message)
